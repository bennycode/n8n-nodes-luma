...
- **Routing is checked against the spec, not executed.** n8n's own
  `NodeTestHarness` is not published to npm, and driving `RoutingNode` directly
  means depending on `n8n-core` and hand-building an `ExecuteContext` out of
  twelve arguments, which would break on every n8n release. Instead
  `test/description.test.ts` asserts that every routed URL, method, query
  parameter and body parameter exists in the OpenAPI document. That catches
  typos and endpoints the API does not have; it does not prove n8n's engine
  emits them, so the manual `npm run dev` pass still matters before a release.
...
- [x] Output schemas for every operation, generated by `npm run sync:openapi`
- [x] Test that every query and body parameter is one the endpoint accepts
- [x] Unit tests for the list search and load options methods
